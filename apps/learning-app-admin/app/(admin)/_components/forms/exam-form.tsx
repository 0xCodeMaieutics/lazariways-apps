    ...
    const payload = {
      ...
      estimatedTimeInMinutes:
        data.estimatedTimeInMinutes && data.estimatedTimeInMinutes > 0
          ...
          : undefined,
      waitUntilPassAllowedInSeconds: parseHoursMinutesToSeconds(passCooldown),
      unlockedByExamId:
        ...
        >
          <FieldGroup>
                    ...
                    <>
                      <Input
                        ...
                        onChange={(event) => {
                          ...
                          const parsed = Number(raw)
                          field.onChange(Number.isFinite(parsed) ? parsed : 1)
                        }}
                        onBlur={field.onBlur}
                        name={field.name}
                        ref={field.ref}
            ...
            <Field>
              ...
              />
              <FieldError errors={[formState.errors.passCooldown]} />
            </Field>
            <Field>
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <>
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                      Learners who can open the topic can take this exam without
                      an Unlocked exam grant. Cannot have an unlocker.
                    </p>
                    <FieldError errors={[fieldState.error]} />
                  </>
                )}
            ...
            <Field>
              <FieldLabel htmlFor="unlockedByExamId">Unlocked by</FieldLabel>
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <>
                    ...
                    >
                      ...
                      {selectableUnlockableExams.map((exam) => (
                        <NativeSelectOption key={exam.id} value={exam.id}>
                          {exam.order + 1}. {exam.title}
                          {exam.isAlwaysUnlocked ? " (always unlocked)" : ""}
                        </NativeSelectOption>
                      ))}
                    </NativeSelect>
                    <FieldError errors={[fieldState.error]} />
              ...
              />
              <p className="mt-2 text-xs text-muted-foreground">
                Learners unlock this exam after fully passing the selected exam.
                Leave empty if it is always unlocked or not yet attached.
              </p>
            </Field>
            <Field>
              <Controller
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      <Checkbox
                        ...
                        disabled={isPending}
                      />
                      <Label htmlFor="enabled">Enable exam</Label>
                    </div>
                    <FieldError errors={[fieldState.error]} />
                  </>
                )}
