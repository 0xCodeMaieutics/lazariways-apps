...
const exerciseCreateSchema = exerciseBaseSchema.superRefine((data, ctx) => {
    ...
    if (needsOptions && data.correctOptionIndex.length === 0) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
                'Select at least one correct option for this exercise type',
    ...
    if (
        ...
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
                'At least one correct input is required for this exercise type',
            path: ['correctInputs'],
        })
    }
})
...
export const adminRouter = router({
    ...
    exams: {
        ...
        reorder: adminProcedure
            .input(
                z.array(z.object({ id: z.string(), order: z.number().int() }))
            )
            .mutation(async ({ input }) => {
            ...
            .mutation(async ({ input }) =>
                prisma.exercise.create({
                    data: {
                        ...
                        correctInputs: input.correctInputs.filter((s) =>
                            s.trim()
                        ),
                    },
                })
                ...
                return prisma.exercise.update({
                    ...
                    data: {
                        ...
                        ...(data.allowsMultipleCorrectOptions !== undefined && {
                            allowsMultipleCorrectOptions:
                                data.allowsMultipleCorrectOptions,
                        }),
                        ...(data.correctInputs !== undefined && {
                            correctInputs: data.correctInputs.filter((s) =>
                                s.trim()
                            ),
                        }),
                    },
