...
function detectTopicUnlockCycle(unlockerByTopicId: Map<string, string | null>) {
  for (const startId of unlockerByTopicId.keys()) {
    ...
    while (current !== null) {
      if (visited.has(current)) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "This unlock configuration would create a circular chain",
        })
      }
      visited.add(current)
...
export async function validateAndApplyTopicUnlock(
  ...
  if (unlockedByTopicId !== null) {
    ...
    if (unlocker.minimumCompletedExamsToUnlock === null) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "The unlocker topic must have a minimum completed exam count",
      })
    }
  }
